...
function dj_loading(ctx, width, height, total, current, _) {
  ...
  if (!dj_loading_value("draw_background")) {
    ctx.fillStyle = dj_loading_color("background", "#000000");
  ...
  var logoWidth = Math.round(
    dj_loading_number("logo_width", dj_loading_logo.width || 300)
  );
  var logoHeight = Math.round(
    dj_loading_number("logo_height", dj_loading_logo.height || 300)
  );
  ...
  var logoTop = dj_loading_number(
    "logo_top",
    (height - logoHeight - barHeight) / 2
  );
  ...
  if (!dj_loading_value("draw_logo") && logoWidth > 0 && logoHeight > 0) {
    ...
    ctx.scale(
      logoWidth / dj_loading_logo.width,
      logoHeight / dj_loading_logo.height
    );
  ...
  if (!dj_loading_value("draw_bar")) {
    var barLeft = Math.round(
      dj_loading_number("bar_left", (width - barWidth) / 2)
    );
    var barTop = Math.round(dj_loading_number("bar_top", logoTop + logoHeight));
    var barBackground = dj_loading_color("bar_background", "#000000");
    ...
    window.dj_loading_fill = Math.min(
      (window.dj_loading_fill || 0) + 1,
      barFill
    );
    //
    if (barBorderWidth > 0) {
      ...
      ctx.fillRect(
        barLeft - barBorderWidth,
        barTop - barBorderWidth,
