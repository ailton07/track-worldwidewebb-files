function getMoralisUserToken(appId) {
  try {
    return JSON.parse(window.localStorage.getItem(`Parse/${appId}/currentUser`))
      .sessionToken;
  } catch (error) {
    console.log(error);
    return "";
  ...
  Moralis.authenticate()
    .then((user) => {
      getEnsName(user.get("ethAddress"))
        .then((ensName) => {
          if (ensName) {
            gml_Script_gmcallback_returnAddress(
              "",
              "",
              user.get("ethAddress"),
              ensName
...
const listWhitelist = () => {
  Moralis.Cloud.run("listWhitelist")
    .then((result) => {
      console.log(result);

      function getList(whitelisted) {
        return result
          .filter((r) => r.whitelisted == whitelisted)
          .map((wl) =>
            wl.ens ? wl.ens : wl.ethAddress.substring(0, 20) + "..."
          );
      }
      ...
      gml_Script_gmcallback_returnWhitelist(
        "",
        "",
        JSON.stringify(whitelist),
        ""
      );
      gml_Script_gmcallback_returnRaffleList(
        "",
        "",
...
const getWhitelistWinners = () => {
  Moralis.Cloud.run("getWhitelistWinners")
    .then((result) => {
      console.log(result);

      function getList() {
        return result.map((wl) =>
          wl.ens ? wl.ens : wl.ethAddress.substring(0, 20) + "..."
        );
      }
...
const getWhitelistResult = async (eventId) => {
  try {
    ...
    gml_Script_gmcallback_returnWhitelistResult(
      "",
      "",
      eventId,
      response.slice(0, 8)
...
const getAavegotchi = async (id, player) => {
  ...
  let callback = player
    ? gml_Script_gmcallback_returnGotchiSprite
    : gml_Script_gmcallback_returnGotchiSpriteUser;
...
const lolyouhackedtheconsolebro = () => {
  Moralis.authenticate().then((user) => {
    console.log(user);
    Moralis.Cloud.run("joinWhitelist")
      .then((result) => {
        console.log(result);
      })
      .catch((error) => {
...
const forcedWLJ = () => {
  Moralis.authenticate().then((user) => {
    console.log(user);
    Moralis.Cloud.run("joinWhitelistForced")
      .then((result) => {
...
const getWhitelistStatus = () => {
  function getStatus(user) {
    Moralis.Cloud.run("checkWhitelist")
      .then((result) => {
        console.log(result);
...
const getWhitelistStatus2 = () => {
  function getStatus(user) {
    Moralis.Cloud.run("checkWhitelist2")
      .then((result) => {
        console.log(result);
...
const littlesChristmasPartyAttendance = () => {
  function littlesAttend(user) {
    Moralis.Cloud.run("littlesChristmasPartyAttendee")
      .then((result) => {
...
const getHighscore = () => {
  function getScore(user) {
    Moralis.Cloud.run("getUserGameHighscore", {
      gameId: "hgjz4gYeZzoyKrx9B83dzCiS",
    })
      .then((result) => {
...
const getTopHighscores = () => {
  function getTopScores(user) {
    Moralis.Cloud.run("getGameHighscores", {
      gameId: "hgjz4gYeZzoyKrx9B83dzCiS",
    })
      .then((result) => {
        console.log(result);
...
const submitHighscore = (score) => {
  function submitScore(user, score) {
    Moralis.Cloud.run("submitGameHighscore", {
      score: score,
      gameId: "hgjz4gYeZzoyKrx9B83dzCiS",
    })
      .then((result) => {
